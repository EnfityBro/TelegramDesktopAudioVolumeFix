# TelegramDesktopAudioVolumeFix

> [!NOTE]
> Automatic volume increase of the system during audio playback in Telegram Desktop.<br>
> It's useful if you have problems with the volume when listening to voice messages or video messages in Telegram Desktop.


## Requirements
- Windows 10+
- 2.7 Mb free space


## How to use
1. Download actual zip archive from [releases](https://github.com/EnfityBro/TelegramDesktopAudioVolumeFix/releases).
2. Unzip to any convenient folder on your computer.
3. Launch 'TelegramDesktopAudioVolumeFix.exe' (or put it in autorun).


## Future plans
- Configure the app folder.
- Add custom settings.
- Add tray icon.
